...
class labels {
    public: 
        ...
        virtual string comb() { return "ret"; };
...
class PushscalClass:public labels{
    public:
        string comb(); PushscalClass(string _op, string _control):labels(_op) { control = _control; opcode = OP_PUSHSCALAR;  dd = -1; };
        string control; int dd = -1;
};
class JumpClass:public labels{
    public:
        string comb(); JumpClass(string _op, string _label):labels(_op) { label = _label; opcode = OP_JUMP;  dd = -1; };
        int dd; string label;
};
class PusharrClass: public labels{
    public:
        string comb(); PusharrClass(string _op, string _control):labels(_op) { control = _control; opcode = OP_PUSHARRAY;  dd = -1; };
        string control; int dd = -1; 
};
class PopscalClass:public labels{
    public:
        string comb(); PopscalClass(string _op, string _control):labels(_op) { control = _control; opcode = OP_POPSCALAR; dd = -1; };
        string control; int dd = -1; 
};
class PoparrClass:public labels{
    public:
        string comb(); PoparrClass(string _op, string _control):labels(_op) { control = _control; opcode = OP_POPARRAY; dd = -1;};
        string control; int dd = -1;
};
...
class GosubClass:public labels{
    public:
        string comb(); GosubClass(string _op, string _label):labels(_op) { label = _label; opcode = OP_GOSUB; dd = -1; };
        int dd; string label;
};
...
class StartClass:public labels{
    public:
        string comb(); StartClass(string _op):labels(_op) { opcode = OP_START_PROGRAM; }; int plt = -1;    
};
...
class JumpZeroClass:public labels{
    public:
        string comb(); JumpZeroClass(string _op, string _label):labels(_op) { label = _label; opcode = OP_JUMPZERO;  dd = -1; };
        int dd; string label;
};
...
class PrintTosClass:public labels{
    public:
        string comb(); PrintTosClass(string _op):labels(_op) { opcode = OP_PRINTTOS; };
};
...
class JumpNZeroClass:public labels{
    public:
        string comb(); JumpNZeroClass(string _op, string _label):labels(_op) { label = _label; opcode = OP_JUMPNZERO; dd = -1; };
        int dd; string label;
};
class PushIClass:public labels{
    public:
        string comb(); PushIClass(string _op, int _pivar):labels(_op) { pivar = _pivar; opcode = OP_PUSHI; }; int pivar;
};
...
string PushIClass::comb(){ return "PushI  (" + to_string(pivar) + ")"; }
